...
int initializeHost(char * input_file)
{
    ...
    /*
     * data from file is transposed so that wraps in kernel are able 
     * to do just 2 transactions instead
     */
    for(uint i = 0; i < n; i++){
        fscanf(ifd, "%d ", &labels[i]);
        for(uint j = 0; j < d; j++){
            fscanf(ifd, "%f ", &train_data[j * n + i]);
        }
    }
    
    for(uint i = 0; i < q; i++){
        for(uint j = 0; j < d; j++){
            fscanf(ifd, "%f ", &test_data[j * q + i]);
        }
    }
...
initializeCL(void)
{
    ...
    // set input variables during compilation
    asprintf(&options ,
...
runCLKernels(void)
{
	...
	*/ 
    ...
    status = clSetKernelArg(
                    kernel, 
                    3, 
                    sizeof(cl_float) * k * (l > group_size ? l: group_size), 
                    NULL);
    if(status != CL_SUCCESS) 
	{ 
    ...
    { 
        std::cout<<
            "Error: eventProfillingInfo: startTime\n";
        return 1;
    }
...
void printArray(cl_float * tab, int h, int w){ 
    for(int i = 0; i < h; i++){ 
        for(int j =0; j < w; j++){ 
            printf("%f, ", tab[i * w + j]); 
        } 
...
main(int argc, char * argv[])
{
    ...
    std::cerr   << "Data transfer + kernel worked for: " 
